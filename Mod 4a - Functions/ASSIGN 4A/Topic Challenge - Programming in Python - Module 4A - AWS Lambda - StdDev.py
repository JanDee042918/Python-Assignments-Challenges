...
p = {"nums": [1, 22, 77, 31, 25]}

# invoke the function with input and fetch result
response = lambda_client.invoke(FunctionName="average", Payload=json.dumps(p))
...
print(f"The standard deviation of nums {p['nums']} is: {float(payload['result']):.3f}")
